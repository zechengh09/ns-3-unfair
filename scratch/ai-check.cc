...
double desiredTputMbps = 9999999999999.;

void updateAckPeriod () {
  // Configure the amount by which to delay each ACK.
  auto period = Seconds ((PACKET_SIZE * 8.) / (desiredTputMbps * 1e6));
  std::cout << "period: " << period.GetMicroSeconds() << " (us)\n";
...
const double BBR_PRINT_PERIOD = 1; // sec
...
void printBbrStats(Ptr<PacketSink> p_sink) {
      ...
      printf("\n");
      printf("Total BBR Packets: %lu\n", p_sink->GetTotalBbrPackets());
...
int main (int argc, char *argv[])
{
  ...
  uint32_t que = 100;
  double durS = 20;
  double delayToAckDelay = 5; // seconds
  bool pcap = false;
  std::string mdl_flp = "";
  std::string out_dir = ".";
  ...
  cmd.AddValue ("desired_tput_Mbps", "Period between ACKs (us).", desiredTputMbps);
  ...
  NS_LOG_INFO ("Desired throughput (Mbps)" << desiredTputMbps);
  ...
  Config::SetDefault ("ns3::TcpSocketBase::AckPeriod", TimeValue (Seconds (0)));
  for (auto* sock : tcpSocketBases) {
  ...
  */
  ...
  Simulator::Schedule( Seconds (delayToAckDelay), updateAckPeriod);
